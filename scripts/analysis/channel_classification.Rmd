...
We also repeat this exercise, but weighting each channel but the number of times we visited it in our crawls. Somewhat surprisingly the alternative classification looks worse with this metric. 
...
Now we look at what share of politics-related channels we are classifying. The big-picture messsage is that our base classification works very well with the politics-related channels.
...
politics_channels <- c(alt_politics_channels, base_politics_channels)

class_full %>%
  filter(channel_id %in% politics_channels) %>%
  count(classification, leaning) %>%
  ggplot(aes(leaning, n, fill = leaning)) +
  geom_col() +
  facet_wrap(~classification) +
