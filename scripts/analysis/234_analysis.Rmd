...
version_string_pretty <- str_to_title(str_replace(params$analysis_dir, 'analysis_', ''))

```

---
title: "Project Analysis for `r version_string_pretty` Scrape Results"
---
...
# video degrees
degrees <- read_csv(here::here(indir, params$analysis_dir, 'vertex_degrees.csv'))

# pageranks
prs <- read_csv(here::here(indir, params$analysis_dir, 'pageranks.csv'))
...
stats_by_depth %>%
  ...
  scale_x_continuous(breaks=1:stats_by_depth$depth)
...
degrees %>%
  ...
  top_n(10, in_deg) %>%
  arrange(-in_deg) %>%
  left_join(video_info %>% select(video_id, title, channel_name), by = "video_id") %>%
  kable(format = "latex",
...
degrees %>%
  ...
  select(-category, -channel_name, -description, -postdate, -title, -out_deg) %>%
...
channel_leanings <- readr::read_csv(here::here(indir, params$analysis_dir, 'channel_classification.csv'))
...
# join the query and channel classifications into the search info dataframe
if (!params$queries) {
  search_w_info %>%
    mutate_at("channel_name", tolower) %>%
    left_join(channel_leanings, by = 'channel_name') %>%
    select(search, video_id, depth, channel_name, leaning) -> search_leanings
} else {
  search_w_info %>%
    mutate_at("channel_name", tolower) %>%
    left_join(channel_leanings, by = 'channel_name') %>%
    left_join(query_classifications, by = 'query') %>%
    select(search, query, video_id, depth, channel_name, leaning, query_leaning) -> search_leanings
}
...
search_leanings %>%
  ...
  count(channel_name, seed_leaning, .drop = FALSE) -> leaning_visit_counts
