...
search_info %>%
  mutate_at("recommendations", format_recs) %>%
  mutate(n_recs = sapply(recommendations, length)) -> search_info
...
This is not necessarily because our recommendation scraper isn't working - we don't follow a recommendation if we've seen it before. Taking a look at number of recommendations we're getting, it appears that this is the case. Our scraper is fine, but the rule of not following videos we have seen before  
